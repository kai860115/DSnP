...
CmdParser::parseCmd(string& option)
{
   ...
   string cmd;
   size_t begin = myStrGetTok(str, cmd);
   if (getCmd(cmd)) {
      begin = str.find_first_not_of(" ", begin);
      size_t end = str.find_last_not_of(" ");
      if (begin != string::npos) {
         option = str.substr(begin, end - begin + 1);
      }
      return getCmd(cmd);
   }
   else {
...
CmdParser::listCmd(const string& str)
{
   ...
   {
      vector<string> _matchedStr;
      vector<string> tokens;
      size_t begin = str.find_first_not_of(' ', 0);
      string token = str.substr(begin, str.find_last_not_of(' ') - begin + 1);
      string s = "";
      size_t n = myStrGetTok(token, s);
      while (s.size()) 
      {
         tokens.push_back(s);
         n = myStrGetTok(token, s, n);
      }
      if(tokens.size() > 1)
      {
         CmdExec* cmd = getCmd(tokens[0]);
         if(_tabPressCount == 1)
         {
            if(cmd)
            {
         ...
         else if(_tabPressCount >= 2)
         {
            if(cmd)
            {
               ...
               else
               {
                  ...
                  if(cmd != 0 && listDir(files, prefix, ".") == 0)
                  {
                     if(files.size() == 1)
                     {
                        size_t i = files[0].size() - prefix.size();
                        string s = files[0].substr(prefix.size(), i) + " ";
                     ...
                     else if(files.size() > 1)
                     {
                        ...
                        char c;
                        bool sameChar = true;
                        for(size_t i = prefix.size(); i < files[0].size(); i++)
                        {
                           c = files[0][i];
                           for(vector<string>::iterator it = files.begin(); it != files.end(); it++)
                           {
      ...
      else
      {
         ...
         {
            bool matched = true;
            string cmdStr = it->first + (it->second)->getOptCmd() + " ";
            if(token.size() <= cmdStr.size())
            {
               for(size_t i = 0; i < token.size(); i++)
               {
                  if(tolower(token[i]) != tolower(cmdStr[i]))
                  {
                     matched = false;
                     break;
                  }
            ...
            if(matched == true) _matchedStr.push_back(cmdStr);
         }
         if(_matchedStr.size() == 0) mybeep();
         else if(_matchedStr.size() == 1)
         {
            if(str[str.size() - 1] != ' ')
            {
               string s = _matchedStr[0].substr(token.size(), _matchedStr[0].size() - token.size());
               for(size_t i = 0; i < s.size(); i++)
               {
                  insertChar(s[i]);
               }
         ...
         else
         {
            if(str[str.size() - 1] != ' ')
            {
               cout << endl;
               int i = 0;
               for(vector<string>::iterator it = _matchedStr.begin(); it!= _matchedStr.end(); it++)
               {
...
CmdParser::getCmd(string cmd)
{
   CmdExec* e = 0;
   // TODO...
   for (auto& ele: _cmdMap) {
      if (myStrNCmp(ele.first + (ele.second)->getOptCmd(), cmd, (ele.first).size()) == 0) {
         e = ele.second;
         break;
      }
   }
