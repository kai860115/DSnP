...
public:
   ...
   public:
      iterator() {}
      iterator(vector<Data>* bucket, size_t numBuckets, size_t bucketNum, size_t idx) : _bucket(bucket), _numBuckets(numBuckets), _bucketNum(bucketNum), _idx(idx) {}
      const Data& operator * () const { return _bucket[_bucketNum][_idx]; }
      iterator& operator ++ () { 
         _idx++;
         while (_bucketNum < _numBuckets && _idx == _bucket[_bucketNum].size()) {
            _bucketNum++;
            _idx = 0;
         }
         return (*this); 
      ...
      iterator& operator -- () { 
         ...
         while (_idx < 0) {
            _bucketNum--;
            _idx = _bucket[_bucketNum].size() - 1;
         }
         return (*this); 
      }
      iterator operator -- (int) { 
