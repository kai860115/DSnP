...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      t = toSizeT(t);
      if (t > this->getRemainSize()) return false;
      ret = (T*)_ptr;
      _ptr += t;
      return true;
   }
...
private:
   ...
   T* getMem(size_t t) {
      ...
      // TODO
      MemRecycleList<T>* m = getMemRecycleList(n);
      ret = m->popFront();
      if (ret) {
         #ifdef MEM_DEBUG
         cout << "Recycled from _recycleList[" << n << "]..." << ret << endl;
      ...
      if (!_activeBlock->getMem(t, ret)) {
         ...
         if (remainSize >= toSizeT(S)) {
            ...
            if (rn != 0) {
               _activeBlock->getMem(toSizeT(rn * S + SIZE_T), ret);
            }
            else {
               _activeBlock->getMem(toSizeT(S), ret);
            }
            #ifdef MEM_DEBUG
            cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
            #endif // MEM_DEBUG
         ...
         MemBlock<T>* m = new MemBlock<T>(_activeBlock, _blockSize);
         _activeBlock = m;
         #ifdef MEM_DEBUG
         cout << "New MemBlock... " << _activeBlock << endl;
         #endif // MEM_DEBUG
